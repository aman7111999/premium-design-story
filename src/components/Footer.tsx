    ...
    <footer className="hairline-t mt-32">
      <div className="container-page grid gap-16 py-20 md:grid-cols-12">
        <div className="md:col-span-6">
          <p className="font-display text-4xl leading-tight md:text-6xl">Have a problem worth solving?</p>
          {site?.email && (
            <a href={`mailto:${site.email}`} className="mt-6 inline-flex items-center gap-2 text-lg link-underline">
        ...
        <div className="md:col-span-3">
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {(site?.socials ?? []).map((s) => (
              <li key={s.url}><a href={s.url} target="_blank" rel="noreferrer" className="link-underline">{s.label}</a></li>
            ))}
          </ul>
        </div>
        <div className="md:col-span-3">
          <p className="text-xs uppercase tracking-widest text-[var(--color-muted)]">Navigate</p>
          <ul className="mt-4 space-y-2 text-sm">
